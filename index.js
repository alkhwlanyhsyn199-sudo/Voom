...
<script>function uploadVoom(){const i=document.createElement("input");i.type="file";i.accept="video/*";i.onchange=function(){const video=i.files[0];if(!video)return;const t=document.createElement("input");t.type="file";t.accept="image/*";t.onchange=function(){const img=t.files[0];if(!img)return;const m=document.querySelector("main");m.innerHTML="";const box=document.createElement("div");box.style.padding="20px";const h=document.createElement("h2");h.textContent="نشر فيديو جديد";const preview=document.createElement("img");preview.src=URL.createObjectURL(img);preview.style.width="100%";preview.style.aspectRatio="16/9";preview.style.objectFit="cover";preview.style.borderRadius="12px";const name=document.createElement("p");name.textContent="🎬 "+video.name;const title=document.createElement("input");title.placeholder="عنوان الفيديو";title.style.width="100%";title.style.padding="12px";title.style.boxSizing="border-box";const desc=document.createElement("textarea");desc.placeholder="وصف الفيديو";desc.style.width="100%";desc.style.height="100px";desc.style.boxSizing="border-box";const publish=document.createElement("button");publish.textContent="🚀 نشر الفيديو";publish.onclick=function(){const data={title:title.value||"فيديو جديد",desc:desc.value||"",thumb:preview.src,video:URL.createObjectURL(video)};localStorage.setItem("voomVideo",JSON.stringify(data));alert("تم نشر الفيديو بنجاح ✅");location.reload()};const back=document.createElement("button");back.textContent="← رجوع";back.onclick=function(){location.reload()};box.append(h,preview,name,title,document.createElement("br"),desc,document.createElement("br"),publish,back);m.appendChild(box)};t.click()};i.click()}</script>
